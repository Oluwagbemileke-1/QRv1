...
export async function verifyEmail(token: string): Promise<AuthResponse> {
  const res = await fetch(`${BASE_URL}/users/verify-email/confirm/`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function listAllUsers(search?: string, role?: string): Promise<UserProfile[]> {
  let nextUrl: string | null = `${BASE_URL}/users/list/${(() => {
    const params = new URLSearchParams();
    if (search) params.set("search", search);
    if (role) params.set("role", role);
    const query = params.toString();
  ...
  while (nextUrl) {
    const res = await authorizedFetch(nextUrl);
